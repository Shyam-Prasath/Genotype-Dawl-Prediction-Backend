...
print("Running Random Forest (FIXED PROTOCOL)")
...
# ===========================
# Cross-Population Evaluation (CORRECT)
# ===========================
...
total_runs = len(panels) * (len(panels) - 1) * len(seeds)

with tqdm(total=total_runs, desc="Running RF Experiments") as pbar:
...
panel_df.to_csv(
    os.path.join(project_root, "cross_pop_results_rf_fixed.csv"),
    index=False
)

degradation_df = pd.DataFrame(degradation_results)
degradation_df.to_csv(
    os.path.join(project_root, "genotype_degradation_results_rf_fixed.csv"),
    index=False
)

print("\nSaved Files:")
print(" - cross_pop_results_rf_fixed.csv")
print(" - genotype_degradation_results_rf_fixed.csv")
